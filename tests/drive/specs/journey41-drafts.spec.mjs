...
const SESSION = "00000000-fixture-0000-000000000041";
const FIXTURE = `/?project=-fixture-project&session=${SESSION}`;

test("drafts survive a reload and follow across devices", async ({ browser }) => {
  // Use a persistent context for page A
  const contextA = await browser.newContext();

  // Start from a known state: clear server draft for the key before opening
  await contextA.request.post("/api/draft", {
    data: { key: SESSION, text: "", at: Date.now() },
  });
